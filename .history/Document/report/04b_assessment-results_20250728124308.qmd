...
The large change in scale in runs when 3 to 5 years of data removed is affected by the presence of the recent block on non-trawl landings fleet selectivity that spans from 2020 forward, and corresponds to an increase in mean length index from the non-trawl fishery (@fig-meanlt-nt) and a decrease in non-trawl discard (@tbl-landings). Within this block selectivity is asymptotic, a change from the dome-shaped selectivity of the previous block. 

A retrospective run was conducted that extended that recent block in time, so it would start in 2011 instead of 2020. The model run with the recent block extended (while substantially degrading fits to composition data) did not have vast increase in scale in its retrospective runs when 3 to 5 years were removed.  

The retrospective analysis highlighted the importance of the recent data for the assessment as well as uncertainty in the estimated scale, but did not reveal a systematic pattern.  
