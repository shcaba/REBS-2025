...
For spawner-recruit curve, steepness ($h$) is fixed to the rockfish prior of 0.72. Recruitment variability is set at 0.5 and checked for consistency with the calculated recruitment variability in the model. Recruitment deviations were estimated (not constrained to sum to 0) for the full time series. Given the longevity of these species, information on recruitment going deeper into the time series is considered reasonable. We explored the model configurations when the shorter time series of recruitment deviations was estimated.
